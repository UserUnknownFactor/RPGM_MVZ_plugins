...
//=============================================================================
 /*:
 * @plugindesc Plugin version 1.19
 ...
 *    \nwd[x]   - Writes out weapon x's description.
 *    \na[x]      - Writes out armor x's name.
 *    \nad[x]    - Writes out weapon x's description.
 *    \ns[x]      - Writes out skill x's name.
 *    \nsd[x]    - Writes out skill x's description.
 *    \nsf[x]    - Writes out skill x's first line of description.
 ...
 *    \ia[x]    - Writes out armor x's name including icon.
...
Window_Base.prototype.convertExtraEscapeCharacters = function(text) {
    ...
    // \NWD[n]
    text = text.replace(/\x1bNWD\[(\d+)\]/gi, function() {
        return $dataWeapons[parseInt(arguments[1])].description;
    }.bind(this));
    // \NA[n]
    ...
    // \NAD[n]
    text = text.replace(/\x1bNAD\[(\d+)\]/gi, function() {
        return $dataArmors[parseInt(arguments[1])].description;
    }.bind(this));
    // \NE[n]
    ...
    // \NSD[n]
    text = text.replace(/\x1bNSD\[(\d+)\]/gi, function() {
        return $dataSkills[parseInt(arguments[1])].description;
    }.bind(this));
    // \NSF[n]
    text = text.replace(/\x1bNSF\[(\d+)\]/gi, function() {
        return $dataSkills[parseInt(arguments[1])].description.split(/\r?\n/)[0];
    }.bind(this));
    // \NT[n]
